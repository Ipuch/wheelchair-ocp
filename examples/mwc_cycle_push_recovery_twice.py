...
from wheelchair_utils.phase_transitions import custom_phase_transition_post, custom_phase_transition_pre
...
def prepare_ocp(
    ...
    objective_functions.add(ObjectiveFcn.Mayer.MINIMIZE_TIME, weight=1, min_bound=1.8, max_bound=2.2, phase=0)
    objective_functions.add(ObjectiveFcn.Mayer.MINIMIZE_TIME, weight=0.01, min_bound=2, max_bound=3, phase=1)
    objective_functions.add(ObjectiveFcn.Lagrange.MINIMIZE_CONTROL, key="tau", weight=100, multi_thread=False, phase=2)
    objective_functions.add(ObjectiveFcn.Lagrange.MINIMIZE_CONTROL, key="tau", weight=100, multi_thread=False, phase=3)
    objective_functions.add(ObjectiveFcn.Mayer.MINIMIZE_TIME, weight=1, min_bound=1.8, max_bound=2.2, phase=2)
    objective_functions.add(ObjectiveFcn.Mayer.MINIMIZE_TIME, weight=1, min_bound=2, max_bound=3, phase=3)
    ...
    x_bounds.add("q_u", bounds=bio_model[0].bounds_from_ranges("q", mapping=variable_bimapping_push), phase=2)
    x_bounds.add("qdot_u", bounds=bio_model[0].bounds_from_ranges("qdot", mapping=variable_bimapping_push), phase=2)

    x_bounds.add("q_u", bounds=bio_model[1].bounds_from_ranges("q", mapping=variable_bimapping_recovery), phase=3)
    x_bounds.add("qdot_u", bounds=bio_model[1].bounds_from_ranges("qdot", mapping=variable_bimapping_recovery), phase=3)
    ...
    x_init.add(key="q_u", initial_guess=np.zeros((1,)), interpolation=InterpolationType.CONSTANT, phase=2)
    x_init.add(key="qdot_u", initial_guess=np.zeros((1,)), interpolation=InterpolationType.CONSTANT, phase=2)
    x_init.add(key="q_u", initial_guess=np.zeros((3,)), interpolation=InterpolationType.CONSTANT, phase=3)
    x_init.add(key="qdot_u", initial_guess=np.zeros((3,)), interpolation=InterpolationType.CONSTANT, phase=3)

    # Define control path constraint
    tau_min, tau_max, tau_init = -60, 60, 0
    ...
    variable_bimapping_ocp.add("tau", to_second=[None, None, 0, 1], to_first=[2, 3], phase=2)
    variable_bimapping_ocp.add("tau", to_second=[None, None, 0, 1], to_first=[2, 3], phase=3)
    u_bounds = BoundsList()
    ...
    u_bounds.add("tau", min_bound=[tau_min] * 2, max_bound=[tau_max] * 2, phase=2)
    u_bounds.add("tau", min_bound=[tau_min] * 2, max_bound=[tau_max] * 2, phase=3)
    ...
    u_init.add("tau", initial_guess=[tau_init] * 2, phase=2)
    u_init.add("tau", initial_guess=[tau_init] * 2, phase=3)

    phase_transition = PhaseTransitionList()
    phase_transition.add(custom_phase_transition_post, phase_pre_idx=0)
    phase_transition.add(custom_phase_transition_pre, phase_pre_idx=1)
    phase_transition.add(custom_phase_transition_post, phase_pre_idx=2)
