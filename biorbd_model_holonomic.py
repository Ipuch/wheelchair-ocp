...
class BiorbdModelCustomHolonomic(BiorbdModel):
    ...
    def add_extra_parameter(self, name: str, value: MX):
        self._extra_parameters[name] = value
    ...
    @property
    def parameters(self):
        return vertcat(*[v for v in self._extra_parameters.values()])

    def holonomic_constraints(self, q: MX):
        if self._extra_parameters == {}:
            return vertcat(*[c(q) for c in self._holonomic_constraints])
        else:
            return vertcat(*[c(q, self.parameters) for c in self._holonomic_constraints])

    def holonomic_constraints_jacobian(self, q: MX):
        if self._extra_parameters == {}:
            return vertcat(*[c(q) for c in self._holonomic_constraints_jacobians])
        else:
            return vertcat(*[c(q, self.parameters) for c in self._holonomic_constraints_jacobians])

    def holonomic_constraints_derivative(self, q: MX, qdot: MX):
        if self._extra_parameters == {}:
            return vertcat(*[c(q, qdot) for c in self._holonomic_constraints_derivatives])
        else:
            return vertcat(*[c(q, qdot, self.parameters) for c in self._holonomic_constraints_derivatives])
    ...
    def holonomic_constraints_double_derivative(self, q: MX, qdot: MX, qddot: MX):
        if self._extra_parameters == {}:
            ...
            return vertcat(*[c(q, qdot, qddot, self.parameters) for c in self._holonomic_constraints_double_derivatives])
    ...
    def null_space_matrix(self, q: MX) -> MX:
        """
        Compute the null space matrix, denoted G in Featherstone's book :
        ...
        -------
        MX
            The null space matrix, such as K(q) * G(qu) = 0
        """

        G = MX.zeros(self.nb_independent_joints, self.nb_dependent_joints)
        G[self._dependent_joint_index, :] = self.coupling_matrix(q)
        G[self._independent_joint_index, :] = MX.eye(self.nb_independent_joints)
